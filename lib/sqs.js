...
function createSqsClient(config, bypassProxy) {
...
function createS3Client(config, bypassProxy) {
  config.httpOptions = _.merge(config.httpOptions, {});
...
module.exports = ({ account, queuePrefix = '', queueSuffix = '', defaultVisibilityTimeout = 300, bypassProxy = false }) => {
  const SQS = createSqsClient(clientConfig, bypassProxy);
  const S3 = createS3Client(clientConfig, bypassProxy);
