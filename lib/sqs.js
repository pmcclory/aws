...
const compress = (data) => pool.exec(encode, [data]).then((result) => Buffer.from(result.data));
//const compress = (data) => Promise.resolve(lz4.encode(data));
...
module.exports = ({
  ...
  const SQS = setHttpAgent(clientConfig, bypassProxy, sslEnabled, AWS.SQS);
  const S3 = setHttpAgent(clientConfig, bypassProxy, sslEnabled, AWS.S3);

  promisify(SQS);
  promisify(S3);

  function getQueueURL(name) {
    let proto = sslEnabled ? 'https' : 'http';
    return `${proto}://sqs.${
      AWS.config.region
  ...
  return {
    ...
    extendedSend: ({
      ...
      return compress(payload, { level })
        .then((payload) => {
          console.log('compress done', payload);
